...
public class DividerLine extends RecyclerView.ItemDecoration {
    ...
    private void drawVertical(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            final int right = getDividerSize() == 0 ? left + dip2px(mContext, dividerSize) : left + getDividerSize();
    ...
    private void drawHorizontal(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //child的左边缘(也是分隔线的左边)
            final int left = child.getLeft() - params.leftMargin+dip2px(mContext, paddingSize);
            //child的底边缘(恰好是分隔线的顶边)
            final int top = child.getBottom() + params.topMargin;
            //child的右边(也是分隔线的右边)
            final int right = child.getRight() - params.rightMargin-dip2px(mContext, paddingSize);
            //分隔线的底边所在的位置(那就是分隔线的顶边加上分隔线的高度)
            final int bottom = getDividerSize() == 0 ? top + dip2px(mContext, dividerSize) : top + getDividerSize();
            dividerDrawable.setBounds(left, top, right, bottom);
            //画上去
            dividerDrawable.draw(c);
        }
