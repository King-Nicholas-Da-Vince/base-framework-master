...
public class LoggingInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (!isDebug || builder.getLevel() == Level.NONE) {
            ...
            String bodyString = new String(MySecurityC.decode(responseBody.bytes()));
...
//                || rSubtype.contains("html"))) {
        if(isDebug)Printer.printJsonRequest(builder, request);
//        }else {
...
//        }
        ...
        if (subtype != null && (subtype.contains("json")
            ...
            if(isDebug)Printer.printJsonResponse(builder, chainMs, isSuccessful, code, header, bodyJson, segmentList);
            body = ResponseBody.create(contentType, bodyString);
        } else {
            if(isDebug)Printer.printFileResponse(builder, chainMs, isSuccessful, code, header, segmentList);
            return response;
        }
        return response.newBuilder().body(body).build();
    }
