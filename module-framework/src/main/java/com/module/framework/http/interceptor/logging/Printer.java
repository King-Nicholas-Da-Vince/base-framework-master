package com.module.framework.http.interceptor.logging;
...
import com.module.framework.utils.MySecurityC;
...
class Printer {
    ...
    private static String bodyToString(final Request request) {
        try {
                    ...
                    if (headers != null && headers.size() > 0) {
                        ...
                        if (split.length == 2) {
                            ...
                            if (keys.length > 1 && body1.contentLength() < 1024) {
                                ...
                                if (MySecurityC.debug){
                                    String body=new String(values[1].getBytes());
                                    builder.append(body);
                                }else{
                                    String body=new String(MySecurityC.decodeCfg(Base64.decode(values[1].getBytes(), Base64.NO_WRAP)));
                                    builder.append(body);
                                }
                                builder.append("\n");
                            }
                            ...
                            builder.append(fileKey+"="+fileName);
                            builder.append("\n");
                        }
                    }
                }
            ...
            if (request.url().toString().contains("common/cfg")){
                String body=new String(MySecurityC.decodeCfg(bytes));
                return body;
            }else{
                String body=new String(MySecurityC.decode(bytes));
                return body;
            }
